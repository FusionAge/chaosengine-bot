...
class Life(commands.Cog):
  def __init__(self, client: commands.Bot):
    self.client = client
    print("life.py LOADED!")

  @app_commands.command(
    name="life",
    description="Update your Life")

  async def damage(self, interaction: discord.Interaction,life: int):
    ...
    if result[0] > 0:
      ...
      if len(results) > 0:
        #USER is Registered
        qry = (f"update users set life = {life} where d_uid = %s and d_gid = %s")
        ...
        try:
          ...
          await interaction.response.send_message(f'Life updated to {life}!')
        finally:
          cursor.close()
      else:
...
async def setup(client: commands.Bot) -> None:
  await client.add_cog(Life(client))