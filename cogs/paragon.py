...
class Paragon(commands.Cog):
  def __init__(self, client: commands.Bot):
    self.client = client
    print("paragon.py LOADED!")

  @app_commands.command(
    name="paragon",
    description="Update your Paragon level")

  async def paragon(self, interaction: discord.Interaction,paragon_level: int):
    ...
    if result[0] > 0:
      ...
      if len(results) > 0:
        #USER is Registered
        qry = (f"update users set plvl = {paragon_level} where d_uid = %s and d_gid = %s")
        ...
        try:
          ...
          await interaction.response.send_message(f'Paragon Level updated to {paragon_level}!')
        finally:
          cursor.close()
      else:
...
async def setup(client: commands.Bot) -> None:
  await client.add_cog(Paragon(client))