...
class Cr(commands.Cog):
  ...
  @app_commands.command(
    name="cr",
    description="Update your Combat Rating")

  async def cr(self, interaction: discord.Interaction,combat_rating: int):
    ...
    if result[0] > 0:
      ...
      if len(results) > 0:
        #USER is Registered
        qry = (f"update users set cr = {combat_rating} where d_uid = %s and d_gid = %s")
        ...
        try:
          ...
          await interaction.response.send_message(f'Combat Rating updated to {combat_rating}!')
        finally:
          cursor.close()              
      else:
...
async def setup(client: commands.Bot) -> None:
  await client.add_cog(Cr(client))